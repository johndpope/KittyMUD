...
-(NSArray*) loadFile:(NSString*)path withSchema:(id<KMExtensibleDataSchema>)schema {
	...
	if(!doc)
		return nil;
    NSString* xslt = @"<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>"
                        ...
                        "</xsl:template>"
                        "</xsl:stylesheet>";
    NSData* xdata = [xslt dataUsingEncoding:NSUTF8StringEncoding];
    doc = [doc objectByApplyingXSLT:xdata arguments:nil error:NULL];
    if(!doc || ![doc isKindOfClass:[NSXMLDocument class]])
        return nil;
